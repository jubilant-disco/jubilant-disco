...
schema.static('getMyDictionary', function(userId, userAlbums) {
    ...
    console.log('MYDICT: ', favAlbums, typeof favAlbums);
    return runAggregate(this, queryVar, favAlbums);
})
...
schema.static('getDictionaries', function(userId) {
    ...
    return this.getAlbums(id)
        .then(({ favAlbums }) => {
            let queryVar = { $ne: userId };
            console.log('OTHER DICT: ', favAlbums, typeof favAlbums)
            return runAggregate(this, queryVar, favAlbums)

            // favAlbums.reduce((acc, album) => {
            //     acc.genre.push(album.genre);
            //     acc.artist.push(album.artist);
            //     acc.albumId.push(album.albumId);
            //     return acc;
        ...
        // .then(favAlbums => {
            // let queryVar = { $ne: userId };
            // console.log('OTHER DICT: ', favAlbums, typeof favAlbums)
            // makeAggregate(queryVar, favAlbums);
        // })
})

function runAggregate(User, queryVar, favAlbums) {
    const { genre, artist, albumId } = favAlbums.reduce((acc, album) => {
        acc.genre.push(album.genre);
        acc.artist.push(album.artist);
        acc.albumId.push(album.albumId);
...
schema.static('calculateMatches', function(dictionaries, myId) {
    console.log('CALC: ', dictionaries, myId);
...
// console.log('id in get dictionary: ', id);
// let queryVar = '';
// if (favAlbums) {
//     queryVar = { $eq: id } ;
//     // console.log('albums in get dictionary: ', userAlbums[0]);
// } else {
//     queryVar = {$ne: id};
...
//         })
// }
// console.log('AFTER: ', id, favAlbums);
